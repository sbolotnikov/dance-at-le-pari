...
import {  TDaySchedule,  TEventScheduleArray } from '@/types/screen-settings';
export const useScheduleDate = (events: TEventScheduleArray, nav: number) => {
  const [dateDisplay, setDateDisplay] = useState('');
  const [days, setDays] = useState<Array<TDaySchedule>>();
  ...
  useEffect(() => {
    ...
    let daysArr: Array<TDaySchedule> = [];
    year = parseInt(
      firstDayOfMonth.toLocaleDateString('en-us', { year: 'numeric' })
    );
    for (let i = 1; i <= paddingDays + daysInMonth; i++) {
      const dayString = `${year}-${month + 1 < 10 ? '0' : ''}${month + 1}-${
        i - paddingDays < 10 ? '0' : ''
      }${i - paddingDays}`;
      if (i > paddingDays) {
        daysArr.push({

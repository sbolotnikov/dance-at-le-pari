...
const page: FC<pageProps> = ({}) => {
  ...
  const checkInformation = () =>{
    ...
    if ((/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(email))&&(/\(?([0-9]{3})\)?([ .-]?)([0-9]{3})\2([0-9]{4})/.test(phone))&&(name.length>4))
      {setGoodInfo(true); console.log('good')}
       else{setGoodInfo(false) 
        console.log('not yet')
       }
      ...
      >
        ...
        >
          ...
          >
            ...
            {((session && session.user.telephone!>"") || goodInfo)?<PaymentForm

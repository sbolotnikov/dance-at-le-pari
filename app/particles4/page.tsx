...
const ParticleAnimation = () => {
  ...
  const particlesRef = useRef<SVGGElement[]>([]);
...
// Firework EFFECT needed to be muliply and made indefinate
  ...
  useEffect(() => {
    ...
    const createParticle = (startX: number, startY: number) => {
      ...
      const resetParticle = () => {
        const shape = particleTypes[Math.floor(Math.random() * particleTypes.length)] as 'heart' | 'leaf' | 'star';
  ...
  const toggleParticleType = (type1: any) => {
    setParticleTypes(prev => 
      prev.includes(type1) ? prev.filter(t => t !== type1) : [...prev, type1]
    );
  };

  return (
    <div className='absolute inset-0' style={{ display: 'flex', flexDirection: 'column', alignItems: 'center'}}>
      <svg ref={svgRef} width={windowSize.width} height={windowSize.height} fill={'black'} />
